...
export async function GET(req: NextRequest) {
  ...
  // ── 8b. Compute principal + PU per-session entries for session='sprintQuali' ─
  ...
  const puSessionScores = calculatePowerUnitSessionScores(carSessionData, 'sprintQuali');
  ...
  const existingPrincipalResults: any[] = (calendar.principalResults ?? []).filter(
    (e: any) => e.session !== 'sprintQuali' && e.session != null,
  );
